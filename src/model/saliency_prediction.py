...
Aggregates temporal-concept saliency scores onto the feature patch grid using
...
class SaliencyPrediction(nn.Module):
    """
    Predict last-frame saliency from ConceptCreation temporal concept representations.

    Uses temporal concept representations for coarse patch saliency prediction and
    incoming trajectory aggregation (pi). Refinement and subpatch decoding use the
    last-frame visual concept context map from visual_concept_representation.
...
s_hat_b = sum_a pi_ab * s_tilde_b(a)
    ...
    def _build_visual_patch_context(
        ...
        *,
        ...
        """
        Reshape last-frame visual concept representations to [B, concept_dim, H, W].
        """
        ...
        if B_vis != B:
            raise ValueError(
                f"Visual feature batch {B_vis} does not match temporal batch {B}"
            )
        if T_vis != T:
            raise ValueError(
                f"Visual feature T {T_vis} does not match temporal T {T}"
            )
    ...
    def _compute_temporal_concept_strength(
        self,
        concept_out: Dict[str, Any],
        last_transition_mask: torch.Tensor,
    ) -> Optional[torch.Tensor]:
        """
        Compute a scalar temporal concept activation strength per selected trajectory.

        Important:
        Do NOT sum all concept activations, because softmax activations sum to 1.
        Use max activation/peakiness so the score reflects how strongly temporal
        concepts activate for this patch-pair trajectory.
        """
    ...
    def forward(
        ...
        temporal_concept_strength = self._compute_temporal_concept_strength(
            concept_out,
            last_transition_mask,
        )

        if temporal_concept_strength is None:
            temporal_concept_strength = torch.ones(
        ...
        if self.use_gated_trajectory_head and self.gated_trajectory_saliency_head is not None:
            ...
            strength = temporal_concept_strength.unsqueeze(-1)
        ...
        patch_concept_context = self._build_visual_patch_context(
        ...
        if self.use_feature_refinement and self.feature_refiner is not None:
            ...
            source_evidence_features = self._aggregate_vector_to_patch_grid(
                source_features * temporal_concept_strength.unsqueeze(-1),
                pi,
                batch_idx,
                target_idx,
                B,
        ...
        out: Dict[str, Any] = {
            ...
            "temporal_saliency_logits": final_logits,
            ...
            "temporal_concept_strength": temporal_concept_strength,
            ...
            "trajectory_temporal_saliency_logits": saliency_logits,
            "temporal_delta_logits": delta_logits,
            ...
            "concept_only_temporal_saliency_logits": concept_only_saliency_logits,
...
class MultiScaleSaliencyPrediction(nn.Module):
    ...
    Each stage uses temporal concepts plus that stage's target-frame Video Swin features.
    ...
    def forward(
        ...
        for stage in self.stage_names:
            ...
            stage_logits.append(out_s["temporal_saliency_logits"])
        ...
        if return_details:
            ...
            for key in [
                ...
                "concept_only_patch_saliency_logits",
                "concept_only_temporal_saliency_logits",
                "trajectory_temporal_saliency_logits",
                "gated_trajectory_residual_logits",
